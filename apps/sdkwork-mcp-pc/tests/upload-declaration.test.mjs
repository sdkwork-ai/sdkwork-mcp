...
const DEDICATED_CONSTANTS_PATH = path.join(
  appRoot,
  'packages/sdkwork-mcp-pc-core/src/sdk/uploadDeclaration.ts',
);
 ...
 *
 * A module may factor a value into a shared constant (`source: MCP_UPLOAD_SOURCE`), so this
 ...
 * `undefined` and fails the agreement assertion — which is the intended outcome, because such a
 * value could not be verified against the declaration.
 */
function readDeclaredConstantValues(source) {
  ...
  const values = [];
  const entryBlocks = source.split(/\bexport const MCP_[A-Z0-9_]*_UPLOAD(?:_H5)?\s*=\s*\{/);
...
test('upload declaration constants mirror the declaration file', () => {
  const constantsSource = fs.readFileSync(DEDICATED_CONSTANTS_PATH, 'utf8');
  const constants = readDeclaredConstantValues(constantsSource);
...
test('upload call sites import the declaration instead of repeating literals', () => {
  // §18.3: a declared value MUST come from the declaration, not from a duplicated literal.
  const servicePaths = [
    'packages/sdkwork-mcp-pc-core/src/sdk/driveAssetUploadService.ts',
  ];
  for (const relativePath of servicePaths) {
    ...
    assert.ok(
      source.includes('MCP_SERVER_ASSET_UPLOAD') || source.includes('MCP_SERVER_ICON_UPLOAD'),
      `${relativePath} does not consume the declared upload constants`,
    );
  }
});
